...
import {
  Swords, Play, BarChart3, Database, FileText, Trophy,
} from 'lucide-react'
import {
  Bar, BarChart, CartesianGrid, Cell, ResponsiveContainer, Tooltip, XAxis, YAxis,
} from 'recharts'
import { api } from '../lib/api'
import { ErrorNote, Field, PageHeader, Spinner } from '../components/Shared'
  ...
  return (
    <div>
      <PageHeader
        ...
        icon={Swords}
      >
        <button className="btn-primary shadow-glow-matrix" onClick={run} disabled={busy}>
        ...
        <div className="space-y-5">
          <div className="card space-y-4">
            <div className="flex items-center justify-between border-b border-cyber-800/80 pb-3">
              ...
              <span className="chip-matrix text-[11px] font-mono">Substitution</span>
            </div>
            ...
            <Field label="Passage Length Limit" hint="(short texts test algorithm bounds)">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-bold uppercase text-slate-400 font-display">Character Cutoff</span>
                <span className="font-mono text-xs font-bold text-matrix-400">{length} chars</span>
              </div>
              <input
                type="range"
                min="60"
                ...
                className="w-full accent-matrix-500 cursor-pointer h-2 bg-cyber-950 rounded-lg border border-cyber-800"
              />
            </Field>
            ...
            <div className="rounded-xl border border-cyber-800 bg-cyber-950/60 p-3.5 text-xs text-slate-400 leading-relaxed space-y-1">
          ...
          {puzzle && (
            <div className="card space-y-2">
              ...
              <div className="font-mono text-xs max-h-36 overflow-auto break-words rounded-xl border border-cyber-800 bg-cyber-950 p-3 text-matrix-300 leading-relaxed glow-text-matrix">
                {puzzle.ciphertext}
              </div>
            </div>
          )}
        ...
        <div className="space-y-5">
          {rows ? (
            <>
              ...
              <div className="card space-y-4">
                ...
                <div className="h-56 w-full rounded-xl border border-cyber-800 bg-cyber-950/90 p-2">
                    ...
                    <BarChart data={chartData} margin={{ top: 10, right: 15, bottom: 5, left: -20 }}>
                      ...
                      <Bar dataKey="accuracy" radius={[6, 6, 0, 0]}>
                        {chartData.map((row, index) => (
                          <Cell
                            key={index}
                            fill={row.accuracy >= 95 ? '#00ff87' : row.accuracy >= 50 ? '#f97316' : '#f43f5e'}
                          />
                        ))}
                      </Bar>
                    </BarChart>
              ...
              <div className="card space-y-4 overflow-x-auto">
                ...
                <table className="w-full text-xs border-collapse">
                  ...
                  <tbody>
                    {rows.map((row) => (
                      <tr key={row.solver} className="border-b border-cyber-800/60 font-mono">
                          ...
                          <>
                            <td
                              className={`p-2.5 text-right font-bold ${
                                row.accuracy >= 0.95
                                  ? 'text-matrix-400 glow-text-matrix'
                                  : row.accuracy >= 0.50
                                    ? 'text-solar-400'
                                    : 'text-rose-400'
                              }`}
              ...
              <div className="card space-y-4">
                ...
                <div className="space-y-3">
                  {rows
                    .filter((r) => !r.error)
                    .map((row) => (
                      <div key={row.solver} className="rounded-xl border border-cyber-800 bg-cyber-950 p-3 space-y-1.5">
                        <div className="flex items-center justify-between text-xs">
                          <span className="font-bold font-display text-slate-200">{row.label}</span>
                          <span className="font-mono text-[11px] text-slate-500">
                            Key: <span className="text-matrix-400 font-bold">{row.key}</span>
                          </span>
                        </div>
          ...
          {history?.summary?.length > 0 && (
            <div className="card space-y-3">
              <div className="flex items-center justify-between border-b border-cyber-800/80 pb-3">
                <h2 className="text-xs font-bold uppercase tracking-widest text-slate-300 font-display flex items-center gap-2">
                  <Database className="h-4 w-4 text-matrix-400" />
                ...
                <tbody>
                  {history.summary.map((row) => (
                    <tr key={row.solver} className="border-b border-cyber-800/60 font-mono">
                      ...
                      <td className="p-2 text-right font-bold text-matrix-400">
