  ...
  return (
    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-800/80 pb-5">
      <div>
        ...
        {blurb && <p className="mt-2 max-w-3xl text-sm leading-relaxed text-zinc-400">{blurb}</p>}
      </div>
...
export function Stat({ label, value, sub, tone = 'default', icon: Icon }) {
  const tones = {
    default: 'text-zinc-100 border-zinc-800/80 bg-zinc-950/60',
  ...
  return (
    <div className={`rounded-xl border p-3.5 backdrop-blur-md transition-all duration-200 ${tones[tone]}`}>
      <div className="flex items-center justify-between">
        <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 font-display">
          {label}
        </div>
...
export function Spinner({ label = 'computing' }) {
  return (
    <span className="inline-flex items-center gap-2 text-xs font-medium text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1.5 rounded-full">
    ...
    <div className="grid grid-cols-[repeat(13,minmax(0,1fr))] gap-1.5">
      {LETTERS.split('').map((cipherLetter, index) => {
        ...
        const tone =
          correct === null
            ? changed
              ? 'border-indigo-500/60 bg-indigo-500/20 text-indigo-200 shadow-sm shadow-indigo-500/30'
              : 'border-zinc-800 bg-zinc-950/90 text-zinc-200'
            : correct
        ...
        return (
          ...
          >
            <div className="text-[9px] font-bold uppercase tracking-wider text-zinc-500 font-display">
              {cipherLetter}
            </div>
...
export function LiveText({ text, previous, truth, limit = 900 }) {
  if (!text) {
    return <div className="font-mono text-xs text-zinc-600 italic">Waiting for backend solver telemetry stream…</div>
  }
  ...
  return (
    <div className="font-mono text-xs leading-relaxed tracking-wider break-words">
      {shown.split('').map((character, index) => {
        ...
        else if (correct === false) className = 'text-zinc-500'
        if (changed) className += ' bg-indigo-500/30 text-indigo-200 rounded px-0.5 animate-pulse'
        return (
      ...
      {text.length > limit && <span className="text-zinc-600 font-sans italic"> … (+{text.length - limit} more characters)</span>}
    </div>
  )
}
