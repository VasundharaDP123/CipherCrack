...
import {
  Trophy, Bot, Clock, Award, CheckCircle2, Save, RefreshCw, Zap,
} from 'lucide-react'
    ...
    <div>
      ...
      >
        <div className="flex gap-2.5">
          ...
          <button className="btn-secondary shadow-glow-solar" onClick={() => newGame()}>
      ...
      {!puzzle ? (
        <div className="card flex flex-col items-center justify-center py-20 text-center text-slate-500 space-y-3">
          <div className="h-12 w-12 rounded-full bg-cyber-950 border border-cyber-800 flex items-center justify-center text-slate-600">
            <Trophy className="h-6 w-6 text-solar-400" />
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr,360px]">
          ...
          <div className="space-y-5">
            <div className="card space-y-4">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <Stat label="Your Time" value={`${elapsed.toFixed(1)}s`} icon={Clock} tone="warn" />
                <Stat
                  label="Your Accuracy"
              ...
              {finished && (
                <div className="rounded-xl border border-matrix-500/40 bg-matrix-500/10 p-4 text-xs font-semibold text-matrix-300 shadow-glow-matrix flex items-center gap-3">
                  <CheckCircle2 className="h-5 w-5 text-matrix-400 shrink-0" />
                  <div>
                    Puzzle solved in <strong className="text-slate-100">{finished.seconds.toFixed(1)}s</strong>!
                    {aiResult &&
              ...
              <div>
                ...
                <div className="font-mono text-xs max-h-36 overflow-auto break-words rounded-xl border border-cyber-800 bg-cyber-950 p-4 text-solar-400 leading-relaxed glow-text-solar">
                  {puzzle.ciphertext}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="font-mono text-xs max-h-36 overflow-auto break-words rounded-xl border border-cyber-800 bg-cyber-950 p-4 leading-relaxed">
                      ...
                      className={
                        ch === '·'
                          ...
                          : puzzle.plaintext[i] === ch
                            ? 'text-matrix-400 font-bold glow-text-matrix'
                            : 'text-rose-400'
                      }
                    >
                      {ch}
            ...
            <div className="card space-y-4">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-slate-300 font-display flex items-center justify-between border-b border-cyber-800/80 pb-2.5">
                  <span>1. Select Ciphertext Letter</span>
                  {selected && <span className="text-solar-400 font-mono text-[11px]">Selected: [{selected}]</span>}
                </div>
                  ...
                  {cipherLetters.map((letter) => (
                    <button
                      ...
                      className={`font-mono text-xs rounded-lg border px-2.5 py-1.5 transition-all cursor-pointer ${
                        selected === letter
                          ? 'border-solar-500 bg-solar-500/20 text-solar-300 font-bold shadow-glow-solar'
                          : mapping[letter]
                            ? 'border-neon-violet/30 bg-neon-violet/10 text-purple-200 font-semibold'
                            : 'border-cyber-800 bg-cyber-950 text-slate-400 hover:border-cyber-700 hover:text-slate-200'
                      }`}
                      title="Double-click to clear mapping"
                    >
                      {letter}
                      <span className="ml-1 text-[10px] text-solar-400 font-bold">
                        →{mapping[letter] || '·'}
                      </span>
                    </button>
                  ))}
              ...
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-slate-300 font-display border-b border-cyber-800/80 pb-2.5">
                    ...
                    <button
                      ...
                      className={`font-mono text-xs rounded-lg border px-3 py-1.5 transition-all disabled:opacity-30 disabled:cursor-not-allowed ${
                        usedPlain.has(letter)
                          ? 'border-neon-violet/30 bg-neon-violet/10 text-purple-300 font-bold'
                          : 'border-cyber-800 bg-cyber-950 text-slate-300 hover:bg-cyber-800 hover:text-white cursor-pointer'
                      }`}
                    >
                      {letter}
                    </button>
                ...
                <p className="mt-2 text-[11px] text-slate-500">
                  Violet letters are already mapped. Double-click a cipher letter to remove mapping.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-5">
            <div className="card space-y-4">
              ...
              <button className="btn-violet w-full py-3" onClick={raceAI} disabled={aiRunning}>
                <Zap className="h-4 w-4" />
                {aiRunning ? 'AI Solving in Progress...' : 'Launch Concurrent AI Solver'}
              </button>
              {aiRunning && <Spinner label={`MCMC step ${aiProgress?.iteration ?? 0}`} />}
              {aiProgress && (
                <div className="font-mono text-xs max-h-24 overflow-auto rounded-xl border border-cyber-800 bg-cyber-950 p-3 text-slate-400 leading-relaxed">
            ...
            {(finished || humanAccuracy > 0.5) && (
              <div className="card-glow space-y-3">
                <div className="text-xs font-bold uppercase tracking-widest text-matrix-300 font-display flex items-center gap-1.5">
                  <Award className="h-4 w-4 text-matrix-400" />
            ...
            <div className="card space-y-3">
              <div className="flex items-center justify-between border-b border-cyber-800/80 pb-3">
                <h2 className="text-xs font-bold uppercase tracking-widest text-slate-300 font-display flex items-center gap-2">
                  <Trophy className="h-4 w-4 text-solar-400" />
                  Global Leaderboard
                </h2>
                <span className="chip-solar font-mono text-[11px]">
                  SQLite Storage
                </span>
              </div>
              {board?.scores?.length ? (
                <table className="w-full text-xs border-collapse">
                  ...
                  <tbody>
                    {board.scores.map((row) => (
                      <tr key={row.id} className="border-b border-cyber-800/60 font-mono">
                        <td className="p-2 font-sans font-semibold text-slate-300">{row.player}</td>
                        <td className="p-2 text-right text-matrix-400 font-bold">
                        ...
                        <td
                          className={`p-2 text-right font-sans capitalize font-semibold ${
                            row.winner === 'human' ? 'text-matrix-400' : 'text-slate-500'
                          }`}
                        >
                          {row.winner}
                        </td>
